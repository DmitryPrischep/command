...
{
    check_key(key_size);
    std::vector<byte_t> password = {0x1F, 0x45, 0xFD, 0xEA, 0x2E, 0x8C, 0x10, 0x02,
                                  0x13, 0x8B, 0xC9, 0x61, 0xA9, 0x9F, 0x35, 0x0A};
//    std::vector<byte_t> password(key_size);
...
byte_t AES::get_r_con(size_t x, size_t y)
{
    return r_con[x][y];
}
...
std::vector<byte_t> Key::shift_word(const std::vector<byte_t> word)
{
    std::vector<byte_t> result(length_word);
...
std::vector<byte_t> Key::sub_word(const std::vector<byte_t> word)
{
    std::vector<byte_t> result(length_word);
    for (size_t i = 0; i < length_word; i++){
        byte_t x = word[i] % 16;
        byte_t y = word[i] / 16;
        byte_t temp = AES::get_s_element(x, y, true);
        result[i] = temp;
    }
    return result;
}

ByteArray Key::get_encrypt_key(std::vector<byte_t> pass)
{
    ...
    for (size_t i = count_word; i < (count_word * (count_round + 1)); i++){
        std::vector<byte_t> word = words[i - 1];
        if(i % count_word == 0){
