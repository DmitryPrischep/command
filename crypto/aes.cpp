...
byte_t AES::xor_byte(byte_t first, byte_t second)
{
    byte_t res = first ^ second;
    return res;
//    return first ^ second;
}
...
byte_t AES::sub_byte(byte_t value, bool direction)
{
    byte_t i = value / s_box.size();
    byte_t j = value % s_box.size();
...
{
    size_t k = 1;
    std::vector<byte_t> result(block.size());
    for (size_t i = 1; i < result.size(); i++){
        for (size_t j = 0; j < result.size(); j++){
            if(direction){
                result[j] = block[i][(j + k) % 4];
            } else {
                if (j >= k) {
                    result[j] = block[i][(j - k) % 4];
                } else {
                    result[j] = block[i][(result.size() + j) - k];
                }
            }
        }
        for (size_t j = 0; j < result.size(); j++){
...
{
    byte_t mix_sum = 0;
    std::vector<byte_t> result(4);
    for (size_t i = 0; i < block.size(); i++){
        for (size_t j = 0; j < block.size(); j++){
            mix_sum = 0;
            for(size_t k = 0; k < block.size(); k++){
                byte_t temp = mul_galois(get_c_element(j, k, direction), block[k][i]);
...
byte_t AES::mul_galois(byte_t a, byte_t b)
{
    byte_t result = 0;
    byte_t n = 128;
    while(b != 0){
...
byte_t AES::get_s_element(size_t x, size_t y, bool direction)
{
    if(direction)
        return AES::s_box[x][y];
    else
        return inv_s_box[x][y];
}

byte_t AES::get_c_element(size_t x, size_t y, bool direction)
{
    if(direction)
        return c_box[x][y];
    else
...
const std::vector<byte_t> AES::encrypt(const std::vector<byte_t>& buffer)
{
    ByteArray block(4, std::vector<byte_t>(4));
    for(size_t i = 0; i < block.size(); i++){
...
const std::vector<byte_t> AES::decrypt(const std::vector<byte_t>& buffer)
{
    ByteArray block(4, std::vector<byte_t>(4));
    for(size_t i = 0; i < block.size(); i++){
        for(size_t j = 0; j < block[0].size(); j++){
